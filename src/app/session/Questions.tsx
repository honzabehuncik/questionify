  ...
  return (
    <div className="flex-grow flex flex-col overflow-hidden">
      <div className="flex-grow overflow-y-auto pb-24 space-y-4"> 
        {questions.map((q: any) => (
          <div key={q.id} className={`border ${q.isHighlighted ? "border-[#7371FF] bg-[#7371FF]/[.1]" : "border-[#5C5C5C]"} rounded-lg p-4 flex flex-col space-y-2`}>
            <div className="flex items-center space-x-3">
              ...
              <span className="text-md font-medium text-text">Anonymous</span>
              <div className="ml-auto flex items-center space-x-2 justify-end w-full">
                <span className={`text-sm ${votedQuestions[q.id] ? "text-primary" : "text-[#5C5C5C]"}`}>
                  {q.votes}
                </span>
                <button
                  onClick={() => handleVote(q.id)}
                  className={`text-sm ${votedQuestions[q.id] ? "text-primary" : "text-[#5C5C5C]"}`}
                >
